...
const server = new McpServer({
  ...
  instructions: [
    ...
    "TWO DIFFERENT GATES, and confusing them ships broken email: `orbit_generate_brain_gate` emits an offline STRUCTURE gate (does this document hold together — sizes, links, module drift); `orbit_render_gate` / `orbit_qa_email` / `orbit_dark_mode_check` are the RENDER gate (does a human see the right thing — contrast, tap targets, dark mode, clipping). A structure gate reads a document; only a render gate reads the world. An email with white-on-white text and a 20px tap target passes the structure gate cleanly, so never present a green structure gate as ready to send.",
    ...
    "  1. `orbit_bootstrap_brain` — scaffold the brain: operating rules, conventions, stage folders, knowledge stubs, and the retention policy that keeps renders out of git.",
    ...
    "  2b. Assemble the FULL module set and write it to `templates/master-template.html` in the repo. Nothing crosses from Orbit's library to their filesystem on its own, and without a master on disk the drift law below has nothing to check against — it will report UNENFORCED forever.",
    "  3. `orbit_generate_brain_gate` — generate the pre-send gate, parameterised to their limits, container width and master path. This is the difference between a design system and a folder of files. It enforces: module drift against the master (a FAIL, not a judgement call — divergence is legal only as an allowlist line citing a ruling), compose-from-the-master, Gmail-first single tier (unsupported constructs are dropped, not degraded), plus byte-clip, overflow, orphan links and CTA parity — and it runs the statistics gate when `orbit_init_verified_claims` has wired one.",
    "  4. `orbit_scaffold_brain_program` — one framed PRD stub per programme, so AI drafts never build unreviewed.",
    "  5. Push with `orbit_esp_push_template` — it reads the template back and hashes it by default. A 2xx is not evidence; only an `exact` verdict is. Then `orbit_qa_email` + `orbit_dark_mode_check` on the readback, because the gate reads a document and only a render reads the world.",
