...
export function buildOrbitMarkdownHeader({
  ...
  return [
    logoPath ? `![Orbit](${logoPath})` : null,
    logoPath ? "" : null,
    `_${ORBIT_ATTRIBUTION}_`,
    `_${ORBIT_TAGLINE}_`,
...
export function buildOrbitMarkdownFooter() {
  return [
    ...
    "---",
    "",
    `_Generated by Orbit. Internal Orbit artifacts are branded for visibility; production customer-facing email output remains unbranded by default._`
  ].join("\n");
}
...
export function renderOrbitSvgBrandBadge({
  ...
  const fill = theme === "dark" ? "#1b1b1b" : "#ffffff";
  const stroke = theme === "dark" ? "rgba(255,255,255,0.18)" : "rgba(27,27,27,0.14)";
  ...
  return [
    ...
    `<text x="${textX}" y="${textY}" font-size="12.5" font-weight="700" font-family="${escapeXml(
      fontStacks.display
    )}" fill="${text}">${escapeXml(label)}</text>`,
