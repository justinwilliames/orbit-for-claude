...
const PHRASE_RULES = [
  ...
  { pattern: /\bthe\s+(number|part|line|bit|thing|stat|detail|question|word|moment)\s+(that\s+)?(I|we)\s+keep\s+(coming\s+back\s+to|returning\s+to|thinking\s+about)\b/gi, severity: "high", category: "language", label: "The-X-I-keep-coming-back-to reveal", explanation: "'The number I keep coming back to is…' stages a reveal instead of making a claim. It is the same manufactured-revelation move as 'what surprised me most' — it performs a discovery beat and asks the reader to wait for the point. Same family: 'the part that stuck with me', 'the line I keep returning to'.", fix: "State the claim directly and let the emphasis come from the fact. '3,500 businesses signed up in 2 years' beats 'the number I keep coming back to is 3,500'." },
  ...
  // Balanced coda — the shape, not the connector (Justin, 31 Aug 2026).
  // Sibling of "Comma-and clause weld" above, which only catches the comma
  // carrier. These two catch the same rhetorical move riding other punctuation:
  // a clause that does its work, then a payoff that echoes it with a new
  // subject. Added after the rule's own author swapped a comma for a colon and
  // called it fixed.
  { pattern: /[^.!?;:\n]{12,90},\s+and\s+(?:so|neither|nor)\s+(?:did|does|do|is|was|are|has|have|had|will|would|can)\b/gi, severity: "medium", category: "structure", label: "Balanced coda", explanation: "A finished clause, then a coda echoing its grammar with a new subject — 'the calls came back to you, and so did the number'. The second half exists to complete a rhythm, not because the reader needed that fact in that position. A person either starts a new sentence or does not say it.", fix: "Delete everything after the comma and check whether a fact was lost. If not, it was cadence. If so, give it its own sentence." },
  { pattern: /[A-Z][^.!?;:\n]{15,80}:\s+(?:it|they|that|this|he|she)['’]?(?:s|re)?\s+\w+/g, severity: "medium", category: "structure", label: "Colon payoff", explanation: "Setup, colon, then a clause pointing straight back at the subject — 'your number is the exception: it's held until Friday'. Identical move to the comma coda, wearing different punctuation. THE CARRIER IS NOT THE RULE: comma, colon, dash, semicolon and 'not X but Y' all deliver the same balanced payoff, so banning one only relocates the habit.", fix: "Cover everything after the colon. If no fact is lost it was cadence — cut it. If a fact is lost it deserves a full sentence of its own." },
