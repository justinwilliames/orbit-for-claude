...
| Route | Verdict |
|---|---|
| A canvas-write REST API | **Does not exist.** Braze's hosted MCP exposes 71 tools (live-verified 2026-09-08; the 43-function figure was the deprecated local server), and zero of them write a Canvas. Orbit's own `server/braze-canvas.js:149-164` says canvas authoring is dashboard-only. **But see `braze-operator`:** `send_operator_prompt` is an MCP tool that drives the dashboard in Braze's own browser, so it can build a Canvas even though no structured tool can. That is a browser path reached through the MCP, not an API — this skill still owns the case where you need to watch each step. |
