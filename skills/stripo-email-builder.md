...
```
(`target:"email_template"`, `template_name:"<name>"`; `dry_run:true` first — confirm `request_body.email_template_id` is set so it hits `/templates/email/update`). Then VERIFY: `orbit_fetch_braze_template(id)` → same id, body length unchanged-ish (NOT a few KB), new asset present, old absent. `get_email_templates` is paginated at 100 (pass `offset:100` for the tail) — confirm the name count stays 1 (no dup).
