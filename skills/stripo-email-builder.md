...
**MOVE to folder is UI-only — and the way it goes WRONG first:**
...
2. **"Select All" only selects CURRENTLY-RENDERED items, AND it ticks the FOLDER cards too.** In a 40+-email view it grabs ~30 emails + the folders = a partial "Selected: N", never the full set. **Deselect the folder cards** (click their checkboxes) before moving or you nest folders.
3. Since select-all can't reach un-rendered cards, **move in batches**: select the rendered emails (folders deselected) → Move → the moved ones leave the view → Select All again grabs the remainder → Move. (A 42-email view went as 30 + 12.)
...
**⚠️ Folder/total count badges are DENORMALISED and LIE after API deletes and UI moves.** After deleting a folder's emails the badge still read the old count (real content: 0); after moving emails in it over-counted; the top "Emails N" total stayed stale too. **Never trust the badge — verify by OPENING the folder and reading `[id^="entity-"]`** (or confirm the source view is empty). Badges reconcile on a later refresh / fresh session.
