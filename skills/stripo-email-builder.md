...
*Best practice: for CONTENT edits, always regenerate/compose and delete the old version rather than browser-editing — significantly more efficient and less error-prone. This governs CONTENT edits; foldering/tidy-up below is still UI, since there is no folder WRITE API (the tree itself is readable via `orbit_list_stripo_folders`).*

## Do the manual work yourself — never push it back
**Foldering, moving, sorting, renaming, any UI tidy-up is YOUR job — do it to completion via Claude-in-Chrome.** Folder READS are API (`orbit_list_stripo_folders` for the tree + destination IDs, `orbit_list_stripo_emails` to enumerate contents), but there is no folder WRITE API — moves happen in the UI, and the move recipe below is proven and works. NEVER flag foldering/organising as "UI-only / no API / needs the user's hand / too fragile to risk the workspace" and defer it — that reads as laziness and erodes trust faster than the polish saves. If the browser is flaky, drive it **blind via `javascript_tool`/DOM** (a dead screenshot channel does NOT stop clicks or JS — see the CDP-flakiness note below), open a fresh tab to recover, and fight through. The job is done when the manual tail is done, not when the API part is. Best practice: always complete the manual tidy-up yourself rather than handing it back.
...
4. **Export to Braze:** `orbit_export_stripo_email_to_braze`, `dedupe_by_name:true` — UPDATES the existing Braze template with the matching NAME in place (no dupes). `dry_run:true` first to confirm `operation:"update"`. **Times out at ~40 — chunk by ~8.** Idempotent.
   **Any other ESP:** `orbit_export_stripo_email_to_esp` with `platform` (iterable | klaviyo | mailchimp | sfmc | customerio — same CSS merge, same batching). **Customer.io lands as a DRAFT.** Its Design Studio API stores content but cannot publish, so every exported row comes back `status:"ok"` with `published:false` and the publish caveat — say so to the user and tell them to publish it in the Customer.io workspace, because until they do the template cannot send. (Before 2026-08-24 this destination returned `{unsupported}` on a claim that Customer.io had no public template API — that was false, and it was an Orbit build gap, not a vendor limit.)
...
## Workspace management (list + delete + move) — API-first reads, UI moves
LISTING is API now (tools added 2026-07-10): `orbit_list_stripo_emails` (paginated GET /emails — name search, sort, `folder_id` filter that is **RECURSIVE into subfolders**, `without_folder` for root-only), `orbit_list_stripo_folders` (the full recursive folder tree with numeric IDs), `orbit_list_stripo_templates` (template IDs incl. which is the configured master). Also `orbit_get_stripo_limits` (org quota: stored emails+templates vs plan cap, metered export counter) and `orbit_export_stripo_email_html` (compiled send-ready HTML to a local file — **METERED, ~300 exports/period, never loop it**). MOVING to folders is still UI-only — Stripo has no folder WRITE API.

**Get metadata for a KNOWN id (read-only, no UI).** `orbit_get_stripo_email <id>` returns `folderId` (null = root), `createdTime`, `updatedTime`, `name` — enough to confirm an email's home folder and recency for **dedup decisions without driving the grid**. Discovering ids inside folders no longer needs the UI: `orbit_list_stripo_emails` with `folder_id` enumerates them (recursively, subfolders included).

**Enumerate / get IDs (read-only) — `orbit_list_stripo_emails` first; the DOM-scrape below is the legacy fallback and the grid-mechanics primer for the MOVE recipe.** Open the emails page in Claude-in-Chrome, read the DOM: rows are `[id^="entity-<emailId>"]`, first `innerText` line = name, second = date. The grid is **virtualised + card-based** (~30 cards render max). **Folders can NEST** (e.g. `Activation` → `00 Welcome`…`10 M10 Accounting`; `GTMs` → dated GTM subfolders), so a folder may contain only subfolders, not emails. To open a folder: **card `…` (kebab) menu → Open** — do NOT double-click (⚠️ **double-click a folder = inline RENAME**, not open; press Escape, never commit the rename). Direct-nav `/emails/<folderId>` only works with the real numeric folderId revealed in the URL after Open; the checkbox's UUID is NOT it.
