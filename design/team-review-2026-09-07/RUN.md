...
BUILD wave 4 — R5 caveats (3 spawns): ofc 2a5422a (orbit.md opener → deed-first, Nebula a9db9f97f3920498c; repo-description.txt + gh repo edit in Promote, Nova a4c5d63613de38d46), get-orbit f18ce98 (provenance comments + last two 80+ → COUNTS.guides, Nova a46d7fca0d927bf9a). In-seat gate: check OK · evals 50/50 (49 ids) · verify:counts 20/20 · 1439/1439 · pack OK (bundle now carries data/counts.json + data/repo-description.txt, ~400 B — .mcpbignore candidate) · get-orbit tsc 0, test:unit 74/74. **D6 (new, Justin):** `gh repo edit` needs `administration: write` — GITHUB_TOKEN cannot hold it; add a fine-grained PAT as secret ORBIT_REPO_ADMIN_TOKEN, or run once by hand: `gh repo edit justinwilliames/orbit-for-claude --description "$(cat data/repo-description.txt)"`. **New finding (Nebula, wave 4):** before the new opener, orbit.md named neither the render gate nor the lifecycle brain anywhere — neither deed has a routing entry in the protocol-selection sections → iteration 2 R1 (Nebula/Vector). Vector's R5 caveat (/skills 86 vs 79) → queue item 7. Sentinel's (live unchanged) → iteration 2 outcome row after merge.
...
| 4 | Four doors re-sequenced (orbit.md opener added in wave 4) | Echo / Nebula / Nova | 8fa0a0d | e60f8b5 | **shipped** — suite 33 inverted to carry the ruling; Justin's veto at merge (D4) |
