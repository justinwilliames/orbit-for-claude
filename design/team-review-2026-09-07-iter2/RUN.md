...
## CONTRACT
R1 Workflow wf_4d10337b-b53 ×9 — all landed, gate 0 failures (R1-evidence-audit.md). Same nine drones, same §3 triage as iteration 1. R1 Workflow ×9 → gate → R2 direct ×5 (eng a4010d0dca2c71981 · design a7eb78d48ff8bf96b · story a7337fdb055639a4b · iris a582b92e06176834b · cos ae9230638aa168ecc — all landed, gate clean) → R3 Workflow wf_6d8c8a6a-06f ×9 (all landed, gate clean; 1,600,742 tok) → R4 written by the orchestrator — 6 ship-now, 5 queue, 3 defer, D5/D6 amended, D7/D8/D9 new; principles 2 and 3 amended → build wave (6 drones, disjoint files) → R5 Workflow ×9 (reviews HEAD d53fced / 3ea16ea).
R5 (iteration 2) Workflow wf_53c08894-a72 ×9 — 9/9, 0 block, 4 agree / 5 caveat (Atlas: /downloads:77 reach claim unbacked; Pulsar: condition 6 unmet → iteration 3 merge-and-publish or halt; Sentinel: nothing measures the merge — no pull_request workflow; Vector: slot 2 labelled misroute — cut the clause; Voyager: live still 79/83 until merge). Orchestrator re-measured slot 2 at HEAD: email-production-system(29) > template-brain(25) — CONFIRMED. Wave 4 (iteration 2) — COMMITTED ofc c09e01e (Stripo copy, Nova a4009b72fea6e1bfb; orbit.md:455 chain + workflow comment, orchestrator) / get-orbit d32ad2f (slot-2 clause cut → template-brain 30 vs 14, /downloads:77 honest sentence, Nebula abb004c11289e71a2). Gates: ofc check OK · 1449 · evals 50/50 · verify:counts 21/21 · suite 18 17/17 · pack OK; get-orbit tsc 0 · 74/74 · verify:admin 67/67. FINAL-SHIPPING-DECISION.md written. Wave 4 plan was: slot-2 clause cut + /downloads:77 justification (Nebula, get-orbit); Stripo env-var copy at server/stripo-emails.js:258,278 — Atlas's fourth-cycle unclaimed item (Nova, ofc). verify:outcomes first run on iteration 2: 11/46 MISS — all orchestrator presence misses (F29/G7/G10 unnamed; gate commands not quoted verbatim) → RUN.md corrected, FINAL will name the three.
BUILD wave (iteration 2) — COMMITTED ofc d53fced / get-orbit 3ea16ea after in-seat gate — orbit-for-claude: `npm run check` OK · `npm test` 1449/1449 · `npm run evals` 50/50 · `npm run verify:counts` 21/21 · `npm run pack` OK · suite 39 22/22; get-orbit: `npx tsc --noEmit -p tsconfig.json` 0 · `npm run test:unit` 74/74 (Node 24) · `npm run verify:admin` 67/67 at 56/100. Follow-up landed in 3ea16ea: Nebula rewrote 3 whatHappens (a69616e5fc1a69645). Detail: ofc — item 1 doors+router (Nebula a889ce6d05a527b42; suite 39 4-red→22/22; evals 50/50; 33 index rows; 5 door phrases rank-1), item 2 spine index row + --strict + fail-open (Voyager aa41026d239a4c368; 21 checks; suite 72 24/24; --live --strict exits 1 on 79/83), item 3 publish gate + weekly + verify-outcomes (Sentinel ad45aad270f304b7e; .mcpbignore deleted, bundle identical 8462 files; outcomes script: iteration-1 SHAs 21/21, carry-forward ids 28 MISS — the closing round named findings by topic not id), item 6 SVG master (Nova ad5046cb2695a1aa8; 6 paths, mean abs diff 1.665, 1.80% px >32; ships in bundle via assets/); get-orbit — item 4 first-run page + reset + confirmPassword + G14 (Nova a5d44a9f205c5b9f4; SEVEN prompts kept; 6/7 labels hit, slot 2 pending manifest; SKILL_COUNT_OBSERVED emitted), item 5 outcome column + funnel CTE (Voyager a3d6320b9a3ea74ca; verify:admin 67/67, 56/100; COMPLETED_DOWNLOAD filter on 4 public/admin reads). Follow-up: Nebula rewrites 3 whatHappens sentences (slots 4/6/7).
...
### Iteration 2 — R4 ship-now bucket → SHAs (branch team-review/2026-09-07, never pushed)

| # | R4 item | Owner | ofc SHA | get-orbit SHA | Disposition |
|---|---|---|---|---|---|
| 0 | verify:admin fixture hotfix (G1) | Voyager | — | 68b3933 | **shipped** — paired seed + client-d; 56/100 honest |
| 1 | Doors + router: README:3/4, trigger phrases, orbit_render_gate named, manifest rebuilt, suite 39, 33 index rows | Nebula | d53fced | — | **shipped** — 5 door phrases rank-1; evals 50/50 |
| 2 | Spine: index set-diff row, homepage comparator, --strict, REPO_DESCRIPTION equality, fail-open closed | Voyager | d53fced | — | **shipped** — 21 checks; suite 72 24/24 |
| 3 | Publish gate step, .mcpbignore deleted, weekly verify-counts-live.yml, scripts/verify-outcomes.mjs | Sentinel | d53fced | — | **shipped** — bundle identical 8462 files; outcomes script presence-only |
| 4 | First-run page (7 prompts, render-gate demo #1), reset lockout branch, confirmPassword removed, SKILL_COUNT_OBSERVED | Nova (+Nebula prose) | — | 3ea16ea | **shipped** — 6/7 labels hit; slot 2 pending manifest tie-break (now rank-1 after item 1) |
| 5 | downloads.outcome + gate_hit; funnel activated via healthyClientWeeks at week grain | Voyager | — | 3ea16ea | **shipped** — verify:admin 67/67 |
| 6 | assets/icon.svg master | Nova | d53fced | — | **shipped** — mean abs diff 1.665, 1.80% px >32; in bundle via assets/ |

Queue/defer (by ruling): D7 Resend welcome · D8 performance-report merge · seven skill pages · sidecar version guard · generate-icon-dark.py in bundle · release reproducibility (G11) · slot-6 routing tie (ip-warming trigger weight).
