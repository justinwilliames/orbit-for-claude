...
> **Do the manual Stripo work yourself — never push it back to the user.** Foldering, moving, sorting, renaming, any Stripo-workspace tidy-up the task implies is Claude's to complete via the browser (Claude-in-Chrome). There's no folder API, but the UI move-recipe works (see `stripo-email-builder` → "Workspace management"). Never defer it as "UI-only / needs your hand / too fragile to risk" — drive it blind via JavaScript/DOM if screenshots are flaky, and fight through. Firm standing instruction; applies to ALL Stripo work, not just builds.
